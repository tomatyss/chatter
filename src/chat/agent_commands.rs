        ...
        "/agent" => {
                ...
                "on" | "enable" => {
                    if agent.is_none() {
                        ...
                        *agent = Some(new_agent);
                        ...
                        println!("   Available tools: {}", agent.as_ref().unwrap().available_tools().join(", "));
                    } else if let Some(ref mut agent) = agent {
                        agent.set_enabled(true);
                        println!("🤖 {} Agent mode re-enabled!", "AGENT:".bright_green().bold());
                    }
                }
                "off" | "disable" => {
                    if let Some(ref mut agent) = agent {
                        agent.set_enabled(false);
                        println!("🤖 {} Agent mode disabled.", "AGENT:".bright_yellow().bold());
                    } else {
            ...
            "history" => {
                ...
                "clear" => {
                    if let Some(ref mut agent) = agent {
                        agent.clear_history();
                        println!("🤖 {} Tool execution history cleared.", "AGENT:".bright_green().bold());
                    } else {
                        println!("❌ Agent mode is not initialized.");
                    }
                }
                ...
                "config" => {
                    if let Some(ref agent) = agent {
                        ...
                        println!("   Enabled: {}", if config.enabled { "Yes".bright_green() } else { "No".bright_red() });
                        println!("   Max file size: {} bytes", config.max_file_size);
                        println!("   Working directory: {}", config.working_directory.display());
                        println!("   Auto backup: {}", if config.auto_backup { "Yes".bright_green() } else { "No".bright_red() });
                        println!("   Dry run mode: {}", if config.dry_run_mode { "Yes".bright_yellow() } else { "No".bright_green() });
                        println!("   Allowed extensions: {}", config.allowed_extensions.join(", "));
                    } else {
                        println!("❌ Agent mode is not initialized.");
                    }
                }
...
fn display_agent_help() {
    ...
    println!("   {} - Show tool execution history", "/agent history".bright_blue());
...
pub fn check_task_completion(
    ...
    if let Some(ref agent) = agent {
        if agent.is_enabled() {
            return agent.is_task_complete(recent_messages);
        }
    }
    false
}
